...
function phpSerialize(mixedValue: any) {
    ...
    const _getType = function (inp: any) {
        let match;
        let key;
        let cons;
        let types;
...
export function unserializeSession(input: string) {
    if (!input) return {};
...
export function serializeSession(data: any) {
    const parts: string[] = [];

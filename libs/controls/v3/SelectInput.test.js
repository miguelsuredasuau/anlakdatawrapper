...
test.serial(
    "SelectInput sets the 'value' property and emits a change event when an option is selected",
    async t => {
...
test.serial('default SelectInput has no uid', t => {
    new SelectInput({
        target: t.context
    });

    t.is(t.context.querySelector('select').getAttribute('data-uid'), null);
});

test.serial('SelectInput can have data-uid', t => {
    new SelectInput({
        target: t.context,
        props: { uid: 'foobar' }
    });

    t.is(t.context.querySelector('select').getAttribute('data-uid'), 'foobar');
});
